...
// ntscビデオ表示クラス定義
class TNTSC_class : public GFXcanvas1 {    
  public:
    TNTSC_class(uint8_t mode=SC_DEFAULT,uint8_t spino = 1, uint8_t* extram=NULL);
    void begin(void);  // NTSCビデオ表示開始
  ...
  private:
    TNTSC_class(uint16_t width, uint16_t height, uint8_t mode, uint8_t spino, uint8_t* extram);
