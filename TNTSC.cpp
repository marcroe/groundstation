...
// 画面解像度別パラメタ設定
typedef struct  {
  uint16_t width;   // Screen horizontal dot number
  ...
  uint8_t  flgHalf; // Number of vertical scanning lines (0: normally 1: half)
...
#define NTSC_LINE (262+0)                     // Number of scanning lines (added to 2 for some monitors)
...
TNTSC_class::TNTSC_class(uint8_t mode, uint8_t spino, uint8_t* extram)
   : TNTSC_class( screen_type[mode].width, screen_type[mode].height, mode, spino, extram) 
   {
}

TNTSC_class::TNTSC_class(uint16_t width, uint16_t height, uint8_t mode, uint8_t spino, uint8_t* extram)
   : GFXcanvas1(width, height)
   {
   _screen = mode; //mode <=4 ? mode: SC_DEFAULT;
   ...
   if (extram) {
     ...
     vram = getBuffer(); //(uint8_t*)malloc(_vram_size);  // ビデオ表示フレームバッファ
   }
}
